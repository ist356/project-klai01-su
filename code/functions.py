...
def get_account(name, tagline, key, region):
    accountv1 = f"https://{region}.api.riotgames.com/riot/account/v1/accounts/by-riot-id/{name}/{tagline}?api_key={key}"
...
def get_summoner(encrypted_puuid, key, platform):
    summonerv4 = f"https://{platform}.api.riotgames.com/lol/summoner/v4/summoners/by-puuid/{encrypted_puuid}?api_key={key}"
    response = requests.get(summonerv4)
    response.raise_for_status()
    summonerv4_data = response.json()
    return summonerv4_data

def get_league(encrypted_summonerid, key, platform):
    leaguev4 = f"https://{platform}.api.riotgames.com/lol/league/v4/entries/by-summoner/{encrypted_summonerid}?api_key={key}"
    response = requests.get(leaguev4)
    response.raise_for_status()
    leaguev4_data = response.json()
    return leaguev4_data

def get_match_id(encrypted_puuid, key, region):
    matchid_uri = f"https://{region}.api.riotgames.com/lol/match/v5/matches/by-puuid/{encrypted_puuid}/ids?start=0&count=10&api_key={key}"
    response = requests.get(matchid_uri)
    response.raise_for_status()
    match_id = response.json()
    return match_id

def get_matchdetail(match_id, key, region):
    matchdetail_uri = f"https://{region}.api.riotgames.com/lol/match/v5/matches/{match_id}?api_key={key}"
